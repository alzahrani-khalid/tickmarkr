...
- Board contract: `createLiveStore` accepts graph JSON up to the separate finite `STORE_LIMITS.graphBytes` bound of 16 MiB, including a complete matching 1.8 MiB plan. Journal records remain capped at 1 MiB, with 256 rows / 2 MiB of retained history; graph capacity does not enlarge journal retention. A missing graph is named absent; an oversized, invalid or unreadable graph is named unreadable with its source error and contributes no cached plan. A readable graph with a mismatched definition hash is not comparable. Only a matching graph contributes planned counts, titles, dependencies and declarations; otherwise the board shows journal tasks and an unknown denominator.
- Gate cells: `OperatorStateFold` and `runGateCells` share current evidence with the board. `task-approved` with `release: recheck` resets old cells and verdict labels immediately, without inventing a dispatch. The daemon's actual `phase-start` rows (`gate:<name>`, `judge`, `review`, with gate attribution) establish running; legacy `gate-start` and protocol `gate-phase-start` remain supported. `suite-wait` and `host-degraded` establish queued and retain the count/reason. The daemon names a gate only when exactly one current gate phase is open, never from a previous red or last parallel sibling. Unattributed waits say gate unknown. After admission the daemon emits an attributed `phase-start` to restore running, or `suite-admitted` to clear an unattributed wait without inventing a gate or another battery round. `gate-result` supplies passed, failed, disabled, not-run or unknown; queued Q and running R stay distinct on the board and in run detail. Evidence references always point to the row responsible for the current cell, so a reset cannot inherit a stale failure label.
...
**Attribution contract (OBS-1153).** Merged authors and the last dispatch are separate facts. `task-done` carries `authors` — the distinct producing channels whose patches reached the merged subject, joined by stable patch identity in `subjectAuthors` (`src/run/daemon.ts`) — beside `assignment`, the last-dispatched seat. A seat that only carried earlier work forward authored nothing, so a task authored by A and finished by B projects `authors A` with last dispatch B; a mixed subject projects `A+B`. Engine preserve commits (`preserveWorktree` in `src/run/git.ts`) record their producing attempt in a `Tickmarkr-Producer` trailer and in the `producer`/`producerAttempt` fields of every `worktree-preserved` and dirty-refusal row; the producer is the attempt whose worker last launched into that checkout (`knownProducer`), never the newest dispatch, and it survives refusal recreation and cherry-pick resume. A caller with no known producer records an explicit `unknown`, and legacy preservation without producer evidence projects `unknown` — never inferred from the newest seat. `OperatorStateFold` (`src/run/operator-state.ts`), `status` (`src/cli/commands/status.ts`) and the cockpit board keep `channel` as the last dispatch and show `authors …` only when the merged authors differ from it; a pre-feature `task-done` row carries no `authors` and projects none.
