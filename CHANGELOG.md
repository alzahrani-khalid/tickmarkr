...
No surface may assert more than the runtime enforces. This release closes four places where a policy was announced more confidently than it was applied. `plan`'s fleet lints stop going silent exactly when the fleet is broken: an empty review fleet is now distinguished from a healthy one and named, where before the absence of a reviewer and the presence of a working one produced the same quiet output. Channel pools become role-scoped, and every configured seat — judge, reviewer, consultant — is checked before dispatch rather than only the worker pool, so a seat that cannot be filled is reported at plan time instead of discovered at the gate that needed it. `resume`'s preflight stops refusing on shapes the resumed graph never uses: the deny-intersect-prefer collision check is scoped to the shapes actually present, so a resume is no longer blocked by a conflict that cannot arise in the graph being resumed. And a gateway's `--json` model list becomes readable — model ids are projected through an allowlist that makes provider and cost fields unspellable, with a distinct reason returned for malformed JSON, a wrong path or field, and rejected values, so an empty list explains itself instead of looking like an outage.

The spec template gains a rule that cost this milestone a task: a criterion that pins the SHAPE of a fix must also pin the CONDITIONS under which it runs, or a correctly-shaped fix that runs only sometimes satisfies it. `finally { if (cond) cleanup() }` has the shape and is inert in production. Say UNCONDITIONAL, or name the branch that may not exist. The same entry records the axis trap behind it: enumerating one axis exhaustively is what hides the others, and coverage guarded site-by-site can be defeated wholesale by a single conditional that leaves every enumeration satisfied.

**Scope note — four of five.** The temp-directory leak this milestone was scoped to fix did NOT ship. Its criterion pinned a construct rather than a guarantee and was satisfiable by fixes that do not hold; the task also required editing assertions in files no task owned, making it unsatisfiable as written. It is deferred to v1.88 with its scope corrected — the guarantee stated as *the directory does not outlive the call, on success and on throw* — and widened past its original four sites. Saying so is the point of a release named for surfaces that must not overclaim.

**Verification note.** This milestone's run finished not-green: its own tip verification failed on a 20-second test timeout under extreme machine load, not on an assertion. The released tree was verified independently — build, full suite and lint all exit 0 on a clean worktree at the release commit. The run's recorded red stands as recorded; it is not laundered by the later green, and the two facts are kept apart deliberately.
