...
- **Caught by this release's own public CI (OBS-1080).** The Evidence view's key handler acted on the selection it captured at its last render: in the moment between a drawn frame and the handler's re-subscription, Enter opened the row next to the highlighted one and a second fast arrow lost its step — reachable by hand on a loaded host. The handler now reads the live selection; a no-render regression test pins it. The same defect in the Home view (OBS-1081) is queued for v2.5.8.
...
- **P7 (OBS-1018) — ready tasks are admitted by critical path, and `plan` shows the order (T1).**
- **P1 (OBS-1011 add.1, OBS-1016, OBS-1009) — harvest truth on Orca: an exited-shaped stream on a live terminal falls back to the screen, a nudge is proven by its receipt, a consult is harvested like a judge (T2).** Riders: casBoard lock staleness and crash recovery, the claim wait on injected time, an unclaimed or malformed receipt keeps a tombstone, `classifyHost` rejects a blank handle.
- **P2 (OBS-1023 add.2–4, OBS-1021, OBS-1016 add.1) — a live worker is never reaped: a quota verdict needs banner text, saturation and cpu hold the legs past the window, a reap kills only its own process group, a same-seat reap loop parks (T3).**
- **P3 (OBS-1013 add.3, OBS-1039, OBS-1025 add.2, OBS-1033, OBS-1020) — a review round that produced nothing spends no worker (T4).** A closure mismatch is a no-verdict, an exhausted round is a typed infra result that keeps the carried findings, a silent pane seat is demoted at the first beat, findings carry bounded fingerprints, and a seat with two no-verdicts in one run leaves the rotation.
- **P4 (OBS-1022, OBS-1028, OBS-1034, OBS-1010, OBS-1006, OBS-1007 add.4/5, OBS-1045, OBS-1030, OBS-1013) — zero-spend verbs, honest counters (T5).** `approve --recheck` re-gates committed or littered work with no worker, counters survive a recheck, a pinned task hosts its own review-fix repair, demotions survive resume by role, operator pages stop repeating, cap and prose parks are typed, and an exhausted review round parks infra with no attempt charged.
- **P5 (OBS-1030, OBS-932, OBS-1007) — dirt and cap trips tell the truth at the gate (T6).** Tracked and entry dirt are preserved to a recoverable ref and fail closed; the diff cap is enforced typed, before any judge or reviewer is spent.
- **P6 (OBS-1044, OBS-1042, OBS-1045, CI grader) — the standalone battery reads the manifest, standalone verifies share one repository lease, the classifier is exact, the grader reads unhandled errors (T7).**
- **P9 (OBS-1046) — one Space press edits one reason, and an untouched scope keeps its bytes (T8).**
- **Leg-2 fix confirmed cross-vendor (OBS-1052).** The daemon threads its run-scoped no-verdict tally into every gate round, so the two-strike retirement T4 shipped is live: a seat that answers in prose twice is absent from the next round (fable-authored, codex-reviewed, `verify --base 4f1543cd --author claude-code:fable`).
- **CI fix leg (OBS-1057, OBS-1058), caught by the public CI of this release.** The repository-lease reservation identity was device+inode, which Linux reuses across unlink/recreate, so a stale reclaim could delete another verify's fresh reservation; identity is now device, inode, size and mtime. The CI grader no longer reds a hosted runner's own `[vitest-worker]: Timeout calling` noise (reported as `runner_rpc_timeouts`); any other unhandled error stays RED.
- **Known gaps (v2.5.7).** The vendor-authored review exclusion (`carriedAuthors`, OBS-1033) ships inert by ruling: wiring it parks every carried cross-vendor failover in a two-vendor fleet (OBS-1033 add.1; patch preserved). Two runner fixes are authored but unmerged after four batteries each red on a different flaky file outside the fix: a replayed build provisions the recreated worktree (OBS-1049) and a recheck discards cached red verdicts while a pinned repair stays on the pin (OBS-1055). Measured test defects: a temp fixture sweeper inside the battery (OBS-1054 add.1), the evidence-view export line count (OBS-1056), and `tickmarkr verify` replaying a cached red (OBS-1056). No kill-and-resume test covers the seeded two-strike path (OBS-1052 add.1).

## v2.5.5 — board, fleet, ledger

**v2.5.5** — the cockpit draws the approved board, a deny reaches every alias and every scope is visible and toggleable, the test gate trusts the runner's own report, command leases replace the suite window, a scope request parks typed, a verdict is reused by identity, a driver is refused when its host is absent, and `init --remove` undoes a workspace. Ten tasks from `specs/v2.5.5-board-fleet-ledger.spec.md`, delivered by run 20260912-215149-…0007 (10/10, tip 653524b9, 43.7 h) plus seven Leg-2 fix legs (tip b88f9a9f), assembled as 6c908055.

- **BD-1 (OBS-965 add.2, RULING-231-19 §2, rider OBS-1002) — the Run body draws the approved task board, rail-less when the daemon places it (T1).** The prototype frame, seven cells, the effort bar and the four-bucket green ship in the cockpit; the owner token mounts the board at three widths and the manual rail is retained; combined Actions/confirm key chunks are split as keys.
- **ID-1 (OBS-972, RULING-231-19 §3) — a deny by explicit model id covers the floating alias that serves it (T2).** Routing exclusion resolves an alias to the identity it serves, so a denied model cannot be reached through its alias.
- **FL-1 (OBS-994, RULING-231-19 §3) — every deny scope is visible with its reach and toggleable in the fleet browser, and the writer round-trips the workers scope (T3).** Reach and per-channel reasons come from the exclusion collector over the staged policy; the preview loads the candidate bytes; a Space press edits one entry; untouched scopes keep their bytes.
- **RT-1 (OBS-995, RULING-231-19 §4) — pool `any` rotates across vendors within the equal-cost run and honours the task floor (T4).**
- **VL-1 (QUEUE §H VL-1, OBS-985 lineage) — the test gate's completion is the runner's own report, never a stdout count (T5).** The manifest gate reads the runner's file report; a deficit is a suite-incomplete verdict, not a guess from summary lines.
- **SW-1 (OBS-997, OBS-1000, QUEUE §H VL-4 per RULING-232-01 §2) — command leases replace the suite window, and a startup reap reads the seat's own rows (T6).** One lease at the shell seam serializes every runner and oracle command; a reaped seat is judged by its own journal rows.
- **SA-1 (OBS-989, QUEUE §H VL-5) — a worker's scope request parks typed, and an approval with files amends the task in place (T7).**
- **VC-1 (QUEUE §H VL-2) — a verdict is reused by verification identity, and a reused verdict says so (T8).** Tool-gate verdicts keyed by base, lockfile and command hashes are reused across resume and tip verify, and every reused row names its origin.
- **HD-1 (OBS-1003, RULING-232-01 add.3) — host-driver truth: a configured driver whose host is absent is refused before run-start, the Orca narrator places the board as a split, and the overseer skill is host-neutral (T9).**
- **UN-1 (OBS-1029) — `tickmarkr init --remove` returns a workspace to its pre-init state and refuses while a run is live (T10).** Skills, marker-bounded guidance blocks, the scaffold spec and registered worktrees are removed exactly; a live lock, a git error or an unverified unregistration refuses before anything is deleted.
- **Leg-2 fix legs confirmed cross-vendor.** Seven fixes from the Leg-2 review (astra reviews claude/gemini-authored work, fable reviews codex-authored work; the fix is authored by the other vendor and re-verdicted by the same reviewer): T9 — a lost board is retired only after the live observer's acknowledged stop, and the recovery of a retired board keeps its tombstone; T8 — a signal-only infra result is never stored or reused as a battery red; T6 — every package-manager test spelling takes the command lease, and a nested runner joins its parent's lease instead of waiting; T7 — an approval with files re-checks separability on the amended graph; T1 — the effort bar apportions its 46 cells instead of rounding each segment, and a plain chunk starting with `[` splits as keys; T10 — `init --remove` refuses on any git state it cannot verify, and every git call on the remove path runs through one helper that strips `GIT_DIR`, `GIT_WORK_TREE` and `GIT_COMMON_DIR` under `LC_ALL=C`; T3 — collector-backed reach and reasons, a loader-backed preview, a one-entry Space and untouched deny scopes, with authored reasons carried through the membership funnel and a reachable per-channel ledger.
- **Known gaps (v2.5.6).** T3 residual (OBS-1046): a Space press on a channel that carries both a flat deny and an independent allow exclusion removes both; the universe-bearing writer rewrites an untouched empty flat deny list as a null tombstone; comment-spacing and key-line-comment preservation in the writer. Deferred minors from the reviewers: casBoard's mkdir lock has no staleness recovery and waits on the daemon thread; the board claim wait is a fixed 5 s; reconcile pages every owned-title leaf's scrollback per sweep; the lease classifier `test[\w:.-]*` also matches a script named `test-lint`; a daemon launched under a leased ancestor inherits the lease token; `classifySignalOnlyTest` is duplicated between the daemon and the gate runner; the Orca seat-spawn examples in the overseer skill lack the vendor flags the herdr branch mandates; `classifyHost` accepts a whitespace-only `ORCA_TERMINAL_HANDLE`; an unclaimed or malformed split receipt keeps its reservation with no operator-facing retire path.

## v2.5.4 — ladder truth

**v2.5.4** — the run refuses a repository whose refs it cannot write, the router climbs a tier and says so, the daemon climbs by itself and every dispatch row names the assignment that ran, the reviewer is at least as capable as the author every round, plan advises from climb telemetry and shows every collateral path, the overseer skill carries the restart and read-back laws, the daemon notices a dead cockpit and reopens it, and a broad glob that owns the daemon owns its oracles. Eight tasks from `specs/v2.5.4-ladder-truth.spec.md`, delivered by run 20260912-062109-…0001 (8/8, tip 6eed19b3, three files[] amendments mid-run) plus a Leg-2 fix branch (`fix/v2.5.4-leg2-all`, tip 79aa634f): four re-review materials fixed and cross-vendor confirmed, the deaf daemon board fixed (OBS-965), and a pre-existing multi-key Parks test defect repaired (OBS-1001).

- **RP-1 (OBS-983/984) — the run refuses before run-start when the repository's refs are not writable (T1).** Before `tickmarkr run` or `resume` starts the daemon, one probe in `git.ts` creates and deletes a real ref under `refs/tickmarkr/preflight/` through git itself, resolving a linked worktree to the main repository's git dir; a refusal names the refs path and the remedy and leaves no run directory, no `run-start` and no `run-resume` row.
- **ES-1 (OBS-986) — the router climbs one tier on request and says whether it did (T2).** `climbChannel` returns the cheapest untried live channel strictly above the current tier flagged `climbed: true`, or `nextChannel`'s same-or-higher pick flagged `climbed: false` with the reason; `routing.escalateTier: on | off` (default on) enters the schema, defaults and template.
- **ES-2 (OBS-986/929/987/928) — the daemon climbs by itself below the ownership check, and every dispatch row names the assignment that ran (T3).** A fingerprint cap, repair exhaustion or a second worker-attributed gate-fail climbs once per engagement and journals `tier-escalated`; a refused unowned path or a red naming only unowned paths parks as `authoring` with the files[] hint and never climbs; a pinned task parks naming the held pin; every `task-dispatch` row's provenance names the dispatched channel, the abandoned pin and the skipped channels, and a resume whose graph pins a channel the record did not journals `restore-rerouted`.
- **RF-1 (OBS-922 add.2/3) — the reviewer is at least as capable as the author, every round (T4).** `review.floor` (`worker` or a tier) enters the config; the reviewer floor is the maximum of the author's tier, the task floor, the configured tier and this task's prior reviewers (their journaled dispatch tiers, restored on resume from review and no-verdict rows alike); the row's meta names `reviewerFloor` and `reviewerFloorCause`; a retry never drops a tier.
- **LN-1 (RULING-231-09, OBS-971) — plan advises from climb telemetry and shows every collateral path (T5).** Under routing lints, one advisory per shape that climbed in the prior-run window, with the two tiers, the weighted run count and `consider routing.floors.<shape>: <tier>` — writing nothing; each run's climb is resolved against that run's own graph snapshot. The scope lint's collateral list is uncapped, direct importers first, with the total.
- **SK-1 (OBS-982/984) — the overseer skill carries the restart, read-back, sandbox and seat laws (T6).** On any restart the overseer re-reads its pane id and re-announces it; briefs take the address from the handoff; after `agent start` the seat waits for the banner, delivers with `pane run` and reads the brief's first words back before any deadline; a sandbox that denies `.git` writes reports and stops so the overseer launches the daemon.
- **WB-1 (OBS-988) — the daemon notices a dead cockpit and reopens it (T7).** A stale watch beat, a missing presence or a dead owner pid journals `watch-board-lost` once per loss; the daemon retires the ghost through the driver, reopens through the narrator (a cached same-pane answer is a failed reopen, journaled `watch-board-reopen-failed`), journals `watch-board-reopened` naming the new pane, and stops after three reopens with `boardless: true`.
- **OL-2 (OBS-990/989) — a broad glob that owns the daemon owns its oracles; an owned test names its unowned source (T8).** A files[] glob that matches a shape-oracle source existing in the compiled repository owns its five oracles; a task owning a dedicated test that imports a source no task owns yields `unowned-source-of-owned-test`, advisory, never aborting.
- **Leg-2 (OBS-965) — the daemon-placed board hears keys again.** The cockpit's stdin proxy drained one chunk and returned null on a pointer-only report, which stopped Ink's drain loop and left the tty handle stopped for the life of the process — every later key queued in the kernel. It now drains until empty; every cockpit test mounts on a tty-faithful stdin fake (highWaterMark 0), and a real-pty end-to-end test proves a pointer report followed by Down moves the selection (red on the old code, green now).
- **Leg-2 fixes confirmed cross-vendor.** The diagnostic authoring park no longer charges a metered attempt it declared unchargeable (OBS-547 regression); plan's climb advisory resolves a task id against its own run's graph, never the current spec's; a resumed round restores prior-reviewer tiers from no-verdict rows too; a reopen never names the pane that just died.

## v2.5.3 — parser, resume and seat truth

**v2.5.3** — the reviewer's closure survives whitespace, the resumed run knows its own graph, a dead seat is named
by its banner, the ownership lint knows the shape oracles, and a selected-subset test run is never a suite deficit.
Eight tasks from `specs/v2.5.3-parser-resume-seat-truth.spec.md`, delivered by run 20260911-150716-…0002 (8/8, tip
9ca25dc2) plus a Leg-2 fix (`fix/v2.5.3-leg2`: an anchored shape-oracle glob must also match the source).

- **PT-1 (OBS-976, Leg-2 rider) — a review closure id matches modulo whitespace; the wrap-join capture joins the corpus (T1).** One function decides whether a reviewer's `resolved` or `reraised` id names a carried fingerprint, comparing both sides with every whitespace run removed, and every closure check in the review gate — membership, duplication, coverage of every prior id — routes through it; the passing row's meta records each normalised match beside the verbatim id. The verdict corpus gains the kimi wrap-join capture, reconstructed fro.
- **RT-1 (OBS-978) — the recorded graph hash is the last audited rehash (T2).** `recordedGraphDefinitionHash` returns the `to` of the LAST `graph-rehash` row when one exists and the run-start hash otherwise, so status, plan, the operator fold and the resume guard read one answer through the one comparator; the operator fold drops its own rehash special case and reads the comparator. A plain resume after an audited rehash is accepted on the rehashed graph and refused on any other, and a second `-.
- **RT-2 (OBS-979) — an unchanged tree replays its red; the worker's refusal is a files[] hint (T3).** A repair attempt whose gate subject digest equals the digest on the previous attempt's red gate-result row journals a `gate-replayed` row naming that attempt and each replayed gate, runs no gate command, and takes the same disposition the prior red took; an attempt on a different digest runs the battery as today, and a replayed red still counts toward the fingerprint cap. When a worker's result is not ok and its summ.
- **ST-1 (OBS-980) — an adapter's banner is harness; a seat that authored nothing at the ceiling is silent, demoted, and named by doctor (T4).** The seat-byte grammar accepts an adapter-declared closed list of harness rows at its banner stage, so a capture holding only the dispatch echo, an adapter's launch banner and the identity line measures zero seat-authored bytes; a review seat that reaches the ceiling with zero such bytes is classified `silent`, demoted at that first silence exactly as a launch-never-started seat is, and never re-seated in the run. The.
- **RT-3 (OBS-981, RULING-230-01 Q3, Leg-2 rider) — the daemon never closes over a dispatchable task; the approval window is the ruled floor (T5).** Before the close loop ends, the daemon audits its end condition: a pending task whose dependencies are all done while a slot is free is dispatched, not closed over, and a close attempted over one journals an `end-condition-held` row naming the task and continues the loop; the run-end summary's pending bucket holds only tasks behind a park or a failed dependency. A task released by an approval, a dependency's merge or.
- **OL-1 (§G spec law) — the ownership lint knows the five shape oracles (T6).** A closed table in the ownership checker maps the daemon and the narrator label map to the five shape oracles — run narration, brand surfaces, notify identity, outcome projections, cockpit setup — and a task whose files[] touch a mapped source without owning an oracle yields an `unowned-shape-oracle` finding per missing oracle, naming the source and the oracle; the finding blocks compile exactly as a corroborated unow.
- **SK-1 (OBS-977, §G HERDR) — supervision skills mandate a blocking journal consumer (T7).** The three shipped supervision skills state, in their run-watch guidance, that a gate watch which ends the seat's turn is no watch: the seat keeps a blocking journal consumer — the shipped watcher or a foreground `until grep` on the run's terminal events — re-armed at most every twenty minutes, and never relies on a `Monitor`-only wake. The overseer skill's seat-spawn and Leg-2 recipes carry the HERDR notes: a mission.
- **VT-4 (OBS-985) — a selected-subset test run is never a suite deficit (T8).** The test gate's non-final round runs only the tests covering the diff; `compareToBaseline` today applies `fileCountDeficit` to that selected run against the FULL-suite baseline count, so every first test gate of every task parks INFRA with "runner reported N test files, below baseline 309" where N is exactly the selection size (run-20260911-150716 T1 131, T2 221). The deficit guard applies only to a run of the whole .
