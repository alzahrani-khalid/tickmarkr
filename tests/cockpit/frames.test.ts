...
/* ------------------------------------------------------------------------ */
/* T9 — narrow frames keep the park's blocker reachable; the run frames pin. */
/* ------------------------------------------------------------------------ */

describe("T9 — Run frames", () => {
  test("at 80x24 and at 120x40 the blocker details of a parked task are reachable and the regenerated byte-pinned run frames equal the capture path's bytes, so a narrow frame that drops the blocker or a stale frame fixture fails", async () => {
    const { mkdirSync, mkdtempSync, rmSync, writeFileSync } = await import("node:fs");
    const { tmpdir } = await import("node:os");
    const { graphDefinitionHash, saveGraph } = await import("../../src/graph/graph.js");
    const { validateGraph } = await import("../../src/graph/schema.js");
    const { captureShellOutput } = await import("../../src/tui/cockpit/capture.js");
    const { shellFixture } = await import("../fixtures/cockpit/final/capture-fixture.js");
    ...
    // A parked task whose reason is wider than an 80-column body: every word of it must reach the frame.
    const reason = "review round cap reached: the reviewer upheld two findings about the retry path and the operator must decide whether to waive them";
    const cwd = mkdtempSync(join(tmpdir(), "t9-park-"));
    try {
      const runId = "run-20260921-100000";
      const graph = validateGraph({ version: 1, spec: { paths: ["fixture.md"], hash: "fixture-t9", source: "native" }, tasks: ["T1", "T2"].map((id, i) => ({ id, title: `Task ${id}`, goal: "Synthetic", shape: "implement", complexity: 1, deps: i ? ["T1"] : [], files: [], acceptance: ["fixture"] })) });
      saveGraph(cwd, graph);
      mkdirSync(join(cwd, ".tickmarkr", "runs", runId), { recursive: true });
      const ev = (event: string, data: Record<string, unknown>, taskId?: string) => JSON.stringify({ ts: "2026-09-21T00:00:00.000Z", event, data, ...(taskId ? { taskId } : {}) });
      writeFileSync(join(cwd, ".tickmarkr", "runs", runId, "journal.jsonl"), [
        ...
        ev("gate-result", { gate: "review", pass: false, details: "review red" }, "T1"),
        ev("task-human", { kind: "gate-fail", reason }, "T1"),
      ].join("\n") + "\n");
      for (const [columns, rows] of [[80, 24], [120, 40]] as const) {
        const m = await mountShell(cwd, runId, columns, rows);
        try {
          await m.send("4");
          // The Run leaf pages verdicts; the shell moves its viewport to the leaf's end, where the park panel sits.
          await m.send("\x1b[6~");
          const frame = stripAnsi(m.frame());
          expect(frame.split("\n").length).toBe(rows);
          expect(frame).toContain("PARK / T1");
          const body = frame.split("\n").map((line) => line.replace(/^│|│$/gu, "").trim()).join(" ");
          for (const word of reason.split(" ")) expect(body, `${columns}x${rows} keeps "${word}"`).toContain(word);
          expect(body).toContain("gate-fail · failed gate review");
          expect(body).toContain("blocks T2");
          expect(body).toContain("decisions: waive · uphold · recheck");
        } finally { await m.close(); }
      }
    } finally { rmSync(cwd, { recursive: true, force: true }); }

    // The committed run frames are exactly what regenerate.ts writes from the capture path today.
...
test("test: the regenerated byte-pinned evidence frames at 80x24 and 120x40 equal the capture path's bytes and keep the raw journal section present, so a stale evidence fixture or a dropped raw section fails", async () => {
  const { captureShellOutput } = await import("../../src/tui/cockpit/capture.js");
  const { shellFixture } = await import("../fixtures/cockpit/final/capture-fixture.js");
  const fixture = shellFixture();
  try {
    for (const [columns, rows] of [[80, 24], [120, 40]] as const) {
      const frame = await captureShellOutput({ ...fixture, view: "evidence", columns, rows });
      const bytes = readFileSync(join(import.meta.dirname, `../fixtures/cockpit/final/evidence.${columns}x${rows}.txt`), "utf8");
      expect(bytes).toBe(frame + "\n");
      expect(frame).toContain("RAW JOURNAL");
      expect(frame).toContain("task-dispatch");
      expect(frame).toContain("terminal failure #L4");
      expect(frame).toContain("disabled #L5");
    }
  } finally { fixture.close(); }
}, 30000);
