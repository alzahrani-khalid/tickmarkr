...
describe("GSD must_haves.truths fail closed (v1.86 T11, spec items 20 + 21)", () => {
  ...
  test("an all-string truths list compiles byte-identically to the pre-change graph", () => {
    // The scratch pin was recorded by running the PRE-change compiler. The FIXTURE pin was re-recorded
    // when 07-01-PLAN.md's context ref moved from `.planning/PROJECT.md` (which resolves only inside the
    // private checkout) to the vendored `fixtures/gsd-sample/PROJECT.md`: the fixture's bytes moved, so
    // its spec hash and context array move with it. Compiler behaviour is unchanged — neither pin was
    // derived from a new rule.
