...
export const SUPERVISION_TIERS = ["orchestrator", "overseer", "watch"] as const;
...
export function beatSupervision(repoRoot: string, tier: SupervisionTier): void {
...
export function armSupervision(
  ...
  try { beatSupervision(repoRoot, tier); }
  catch { /* repo gone / disk full — the tier reads ABSENT rather than crashing its watcher */ }
  const timer = setInterval(() => {
    try { beatSupervision(repoRoot, tier); } catch { /* repo gone / disk full — let the beat expire */ }
  }, beatMs);
  timer.unref();
  let stoodDown = false;
  return {
...
function beatMtimeMs(repoRoot: string, tier: SupervisionTier): number | "ABSENT" | "UNREADABLE" {
  let st: Stats;
  try {
...
function beatLiveness(tier: SupervisionTier, mtimeMs: number | "ABSENT" | "UNREADABLE", now: number): TierLiveness {
  if (typeof mtimeMs !== "number") return { tier, state: mtimeMs };
...
function readStandDown(repoRoot: string, tier: SupervisionTier): { mtimeMs: number } | "NONE" | "UNREADABLE" {
  const p = supervisionStandDownPath(repoRoot, tier);
  let st: Stats;
  try {
    st = statSync(p);
  ...
  try {
    const rec = JSON.parse(readFileSync(p, "utf8")) as { tier?: unknown; disarmedAt?: unknown };
...
export const supervisionText = (tiers: readonly TierLiveness[], divider = " · "): string =>
  `supervision: ${tiers.map((t) => `${t.tier} ${t.state}`).join(divider)}`;
