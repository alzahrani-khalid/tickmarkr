...
export function fleetEditableFromConfig(cfg: TickmarkrConfig): FleetEditable {
  ...
  for (const [adapter, entry] of Object.entries(cfg.tiers)) {
    tiers[adapter] = {};
    for (const [model, tier] of Object.entries(entry.models)) tiers[adapter][model] = { tier };
  }
  return {
...
export function repoOverlayYaml(overlay: Record<string, unknown>, provenance: Record<string, Record<string, string>> = {}): string {
...
export function serializeFleetOverlay(
  ...
  if (routing) {
    ...
    if (deny && (deny.adapters !== undefined || deny.models !== undefined)) {
      lines.push("  deny:");
      if (deny.adapters !== undefined) lines.push(...block({ adapters: deny.adapters }, "    "));
      if (deny.models !== undefined) lines.push(...block({ models: deny.models }, "    "));
    }
    ...
    for (const [adapter, entry] of Object.entries(tiers)) {
      ...
      if (models.length) {
        ...
        for (const [model, tier] of models) {
          if (tier === null) body.push(`      ${model}: null`);
          else {
...
export function unifiedYamlDiff(before: string, after: string, label = "config overlay"): string {
  ...
  while (i < a.length || j < b.length) {
    ...
    hunks.push("@@");
    for (let k = startI; k < i; k++) hunks.push(`-${a[k]}`);
    for (let k = startJ; k < j; k++) hunks.push(`+${b[k]}`);
  }
  return `${header.join("\n")}\n${hunks.join("\n")}\n`;
}
